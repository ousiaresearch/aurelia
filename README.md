...
Aurelia pushes live to Cloudflare via `aurelia_cf_pusher.py`:

- per-year snapshots
- chronicle entries
- federation causal events
- cross-world movements
- causal summaries
- world state

The Cloudflare layer is the durable memory. It is what makes Aurelia a real system, not a notebook.

Cloudflare endpoints:

- Worker: `hermes-state-worker.plntrprotocol.workers.dev`
- Project: `cf-worker/` (D1 + R2, R2 for binary snapshots, D1 for indexed queries)
